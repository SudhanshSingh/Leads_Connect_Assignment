  ...
  try {
    ...
    if (!validator.isValidObjectId(userId))
      return res
        ...
        .send({
          status: false,
          message: `This UserId ${userId} is Invalid ! 🙄`,
        });
    ...
    if (!validator.isValidObjectId(cartId))
      return res
        ...
        .send({
          status: false,
          message: `This cartId ${cartId} is Invalid ! 🙄`,
        });
    ...
    if (!cartDoc)
      return res
        ...
        .send({
          status: false,
          message: `No Cart Found with this CartId ${cartId} 🛁`,
        });

    if (cartDoc.items.length === 0)
      return res
        .status(404)
        .send({ status: false, message: "No Product Found in this Cart 🛁 " });
    ...
    cancellable
      ? (cartDoc["cancellable"] = cancellable)
      : (cartDoc["cancellable"] = cancellable);
...
module.exports = { createOrder, orderDetails, getOrderList};
