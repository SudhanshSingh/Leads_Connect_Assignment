...
const createCart = async function (req, res) {
  try {
    ...
    if (!validator.isValidBody(data))
      return res
        .status(404)
        .send({ status: false, message: "plz provide the data" });
    ...
    if (quantity) {
      if (quantity <= 0)
        return res
          .status(400)
          .send({ status: false, message: "quantity must be greater than 0" });
    }
    if (!quantity) {
      quantity = 1;
    }
...
const updateCart = async function (req, res) {
  try {
    ...
    if (!productId)
      return res
        .status(400)
        .send({ status: false, message: "ProductId Is requird !!" });
    ...
    if (!productDoc)
      return res
        ...
        .send({
          status: false,
          message: `No product Exist with this productId (${productId})`,
        });
    ...
    if (!cartId)
      return res
        .status(400)
        .send({ status: false, message: "cartId Is requird !!" });
    ...
    if (!(removeProduct == "1" || removeProduct == "0"))
      return res
        ...
        .send({
          status: false,
          message: "removeProduct  should be 0 or either 1 !!",
        });
    ...
    if (removeProduct == 0) {
      for (let i = 0; i < cartArr.length; i++) {
        if (cartArr[i].productId.toString() === productId) {
          let productPrice =
            cartDoc.totalPrice - cartArr[i].quantity * productDoc.price;
          let passObj = {
            totalPrice: productPrice,
            totalItems: cartDoc.totalItems - 1,
  ...
  try {
    ...
    if (data.items.length === 0)
      return res
        ...
        .send({
          status: false,
          message: "Cart is Empty or already deleted 😒",
        });
    if (!data)
      return res
        .status(404)
...
const deleteCart = async function (req, res) {
  try {
    ...
    const data = await cartModel.findOneAndUpdate({ userId }, newObj, {
      new: true,
    });
